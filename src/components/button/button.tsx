...
const Button: React.FC<IButtonProps> = ({ children, onClick, disabled = false, variant = 'blue' }) => {
    ...
    return (
        <button
            className={`flex items-center justify-center w-[320px] xs361:w-[400px] h-[60px] px-9 py-4 gap-2 rounded-lg font-medium transition-colors duration-200 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed ${colorClasses[variant]}`}
            onClick={onClick}
            disabled={disabled}
        >
            {children}
